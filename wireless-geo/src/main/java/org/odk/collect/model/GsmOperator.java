...
public class GsmOperator {
    ...
    public GsmOperator(final CellIdentityGsm cellIdentG) throws GsmOperatorException {
        ...
        mcc = Build.VERSION.SDK_INT >= 28?cellIdentG.getMccString():cellIdentG.getMcc()+"";
        mnc = Build.VERSION.SDK_INT >= 28?cellIdentG.getMncString():determineMnc(cellIdentG.getMnc(), mcc);

        fcn = Build.VERSION.SDK_INT >= 24 && cellIdentG.getArfcn() != Integer.MAX_VALUE ? cellIdentG.getArfcn() : 0;

        if (NetworkMapManager.DEBUG_CELL_DATA) {
            String res = "GSM Cell:" +
                    "\n\tCID: " + cellId +
                    "\n\tLAC: " + xac +
        ...
        if (!validCellId(this.cellId, "GSM") || !validXac(xac) || !validMccMnc(mcc,mnc)) {
            if (NetworkMapManager.DEBUG_CELL_DATA) {
                if (Build.VERSION.SDK_INT >= 24) {
                    Timber.i("Discarding GSM cell with invalid ID for ARFCN: " + cellIdentG.getArfcn());
                } else {
                    Timber.i("Discarding GSM cell with invalid ID");
                }
            }
    ...
    public GsmOperator(final CellIdentityLte cellIdentL) throws GsmOperatorException{
        ...
        mcc = Build.VERSION.SDK_INT >= 28?cellIdentL.getMccString():cellIdentL.getMcc()+"";
        mnc = Build.VERSION.SDK_INT >= 28?cellIdentL.getMncString():determineMnc(cellIdentL.getMnc(), mcc);

        fcn = Build.VERSION.SDK_INT >= 24 && cellIdentL.getEarfcn() != Integer.MAX_VALUE ? cellIdentL.getEarfcn() : 0;
        if (NetworkMapManager.DEBUG_CELL_DATA) {
            String res = "LTE Cell: " +
                    "\n\tCI: " + cellId +
        ...
        if (!validCellId(this.cellId, "LTE") || !validXac(xac) || !validMccMnc(mcc,mnc)) {
            if (NetworkMapManager.DEBUG_CELL_DATA) {
                if (Build.VERSION.SDK_INT >= 24) {
                    Timber.i("Discarding LTE cell with invalid ID for EARFCN: " + cellIdentL.getEarfcn());
                } else {
                    Timber.i("Discarding LTE cell with invalid ID");
                }
            }
    ...
    public GsmOperator(final CellIdentityNr cellIdentN) throws GsmOperatorException{
        ...
        fcn = Build.VERSION.SDK_INT >= 24 && cellIdentN.getNrarfcn() != Integer.MAX_VALUE ? cellIdentN.getNrarfcn() : 0;
        if (NetworkMapManager.DEBUG_CELL_DATA) {
            String res = "NR Cell: " +
                    "\n\tNCI: " + cellId +
        ...
        if (!validCellId(this.cellId, "NR") || !validXac(xac) || !validMccMnc(mcc,mnc)) {
            if (NetworkMapManager.DEBUG_CELL_DATA) {
                if (Build.VERSION.SDK_INT >= 24) {
                    Timber.i("Discarding NR cell with invalid ID for NRARFCN: " + cellIdentN.getNrarfcn());
                } else {
                    Timber.i("Discarding NR cell with invalid ID");
                }
            }
    ...
    public GsmOperator(final CellIdentityWcdma cellIdentW) throws GsmOperatorException {
        ...
        mcc = Build.VERSION.SDK_INT >= 28 ? cellIdentW.getMccString() : cellIdentW.getMcc() + "";
        mnc = Build.VERSION.SDK_INT >= 28 ? cellIdentW.getMncString() : determineMnc(cellIdentW.getMnc(), mcc);

        fcn = Build.VERSION.SDK_INT >= 24 && cellIdentW.getUarfcn() != Integer.MAX_VALUE ? cellIdentW.getUarfcn() : 0;
        ...
        if (!validCellId(this.cellId, "WCDMA") || !validXac(xac) || !validMccMnc(mcc, mnc)) {
            if (NetworkMapManager.DEBUG_CELL_DATA) {
                if (Build.VERSION.SDK_INT >= 24) {
                    Timber.i("Discarding WCDMA cell with invalid ID for UARFCN: " + cellIdentW.getUarfcn());
                } else {
                    Timber.i("Discarding WCDMA cell with invalid ID");
                }
            }
    ...
    public static String getOperatorName(final String operatorCode) {
        ...
        if (null != operatorCode && operatorCode.length() >= 5) {
            ...
            //DEBUG:  NetworkMapManager.info("Operator MCC: "+mcc+" MNC: "+mnc);
            ...
            if (null != operator) {
                //DEBUG: NetworkMapManager.info("matched operator: "+operator+" ("+mcc+":"+mnc+")");
                return operator;
            }

            NetworkMapManager.State s = NetworkMapManager.getManager().getState();
    ...
    private static boolean recognizedMnc(final String mcc, final String mnc) {
        if (null != mcc && null != mnc && mcc.length() == 3 && mnc.length() > 1) {
            ...
            String operator = mccMap.get(mnc);
            if (null != operator) {
                //DEBUG: NetworkMapManager.info("matched operator L1: "+operator+" ("+mcc+":"+mnc+")");
                return true;
            }
            NetworkMapManager.State s = NetworkMapManager.getManager().getState();
            if ((null != s) && (null != s.mxcDbHelper)) {
                try {
                    ...
                    Timber.w(sqldbex,"Mxc DB corrupt - unable to lookup carrier");
                    //attempt to recopy the MccMnc DB file to see whether we can recover.
                    s.mxcDbHelper.implantMxcDatabase(NetworkMapManager.getManager().getContext(), false);
                }
                if (operator != null) {
                    //DEBUG: NetworkMapManager.info("matched operator L2: "+operator+" ("+mcc+":"+mnc+")");
                    mccMap.put(mnc, operator);
                    return true;
                }
            }
