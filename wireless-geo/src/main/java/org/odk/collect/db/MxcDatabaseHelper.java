...
import static org.odk.collect.util.FileUtility.EST_MXC_DB_SIZE;
...
import org.odk.collect.model.MccMncRecord;
...
public class MxcDatabaseHelper extends SQLiteOpenHelper {
    ...
    public void implantMxcDatabase(final Context context, final Boolean isFinishing){
        Timber.i("installing mmc/mnc database...");
        ExecutorService executor = Executors.newSingleThreadExecutor();
        Handler handler = new Handler(Looper.getMainLooper());
        executor.execute(() -> {
            int installCount = prefs.getInt(PREF_MXC_REINSTALL_ATTEMPTED, 0);
...
//                    }
            ...
            try {
                if (isPresent()) {
                    ...
                    Timber.i("Installing mxc file at: " + outputFile);
                    ...
                    Timber.e("stopped trying to implant Mxc DB: reached max tries.");
                }
            } catch (IOException ioe) {
                Timber.w("Exception installing mxc: " + ioe);
            } finally {
                try {
                    ...
                    Timber.e(nioe,"hopeless Mxc DB implant error: ");
                }
                final SharedPreferences.Editor editDone = prefs.edit();
                editDone.putInt(PREF_MXC_REINSTALL_ATTEMPTED, installCount+1);
                editDone.apply();
            }
        });
    }
    ...
    public MccMncRecord networkRecordForMccMnc(final String mcc, final String mnc) throws SQLException {
        ...
        try {
            if (openDataBase()) {
                ...
                Timber.e("unable to open mcc/mnc database for record.");
            }
        } catch (StackOverflowError soe) {
            Timber.e(soe,"Database corruption stack overflow: ");
    ...
    public String networkNameForMccMnc(final String mcc, final String mnc) throws SQLException {
        ...
        try {
            if (openDataBase()) {
                ...
                Timber.e("unable to open mcc/mnc database for name.");
            }
        } catch (StackOverflowError soe) {
            Timber.e(soe,"Database corruption stack overflow: ");
            throw new SQLiteDatabaseCorruptException("Samsung-specific stack overflow on integrity check.");
    ...
    private boolean openDataBase() throws SQLException {
        ...
        try {
            if (null == db || !db.isOpen()) {
                ...
                Timber.i("trying to open db: " + mxcPath);
